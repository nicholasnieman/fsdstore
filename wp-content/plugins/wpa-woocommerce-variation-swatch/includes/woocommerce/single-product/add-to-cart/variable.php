...
// Get gallery data
$galleries = WPA_WCVS_Frontend::image_galleries( $product->get_id(), $available_variations );
				...
				<?php foreach ( $attributes as $attribute_name => $options ) : ?>
				   ...
				   <div class="swatch is-<?php echo esc_attr( isset( $attr->attribute_type ) ? $attr->attribute_type : $custom_attr_type ); ?>">
						...
						<?php elseif ( $custom_attr_type == 'color' || $custom_attr_type == 'label' ) : ?>
							<ul class="swatch__list is-flex" data-attribute="<?php echo esc_attr( strtolower( $attribute_name ) ); ?>">
								<?php
									foreach ( $options as $attr_value ) {
										$attr_color = get_post_meta( $product->get_id(), 'custom_attr_color_' . str_replace( ' ', '_', $attr_value ), true );
