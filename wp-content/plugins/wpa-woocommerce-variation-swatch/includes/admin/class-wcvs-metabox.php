		...
		if ( 'product' == $post->post_type ) {
			...
			if ( $product->is_type( 'variable' ) ) {
				...
				foreach ( $attributes as $attribute_name => $options ) {
					...
					if ( $attr && ( 'color' == $attr->attribute_type ) ) {
		...
		if ( ! empty( $attribute ) ) {
			foreach ( $attribute as $key => $single_attribute ) {
				if ( isset( $single_attribute ) && ! $single_attribute['is_taxonomy'] ) {
					...
					$display_type = $_POST['_display_type_' . $single_attribute['name']];
					if ( ! empty( $display_type ) ) {
						update_post_meta( $post_id, '_display_type_' . $single_attribute['name'], esc_attr( $display_type ) );
					}

					foreach ( $custom_attrs as $custom_attr ) {
						$custom_attr_color = $_POST['custom_attr_color_' . str_replace( ' ', '_', $custom_attr )];
						if ( ! empty( $custom_attr_color ) ) {
							update_post_meta( $post_id, 'custom_attr_color_' . str_replace( ' ', '_', $custom_attr ), esc_attr( $custom_attr_color ) );
						}

						$custom_attr_img = $_POST['custom_attr_img_' . sanitize_title( $custom_attr )];
						if ( ! empty( $custom_attr_img ) ) {
							update_post_meta( $post_id, 'custom_attr_img_' . sanitize_title( $custom_attr ), esc_attr( $custom_attr_img ) );
						}

						$custom_attr_label = $_POST['custom_attr_label_' . sanitize_title( $custom_attr )];
						if ( ! empty( $custom_attr_label ) ) {
							update_post_meta( $post_id, 'custom_attr_label_' . sanitize_title( $custom_attr ), esc_attr( $custom_attr_label ) );
						}
					}
				}
			}
					...
					if ( isset( $single_attribute ) && ! $single_attribute['is_taxonomy'] ) {
						$custom_attrs = explode( ' | ', $single_attribute['value'] );
							...
							woocommerce_wp_select( 
								array( 
									'id'          => '_display_type_' . $single_attribute['name'],
							...
							$display_type = get_post_meta( $post->ID, '_display_type_' . $single_attribute['name'], true );

							if ( $display_type !== 'default' ) {
								foreach ( $custom_attrs as $custom_attr ) {
								$custom_attr_color  = get_post_meta( $post->ID, 'custom_attr_color_' . str_replace( ' ', '_', $custom_attr ), true );
								...
								echo '<p class="form-field wcvs-default-field">';
									...
									if ( $display_type !== 'label' ) {
										echo '<span class="wcvs-color-field">';
											echo '<input type="text" class="wcvs-color-picker" name="custom_attr_color_' . str_replace( ' ', '_', $custom_attr ) . '" id="custom_attr_color_' . str_replace( ' ', '_', $custom_attr ) . '" value="' . $custom_attr_color . '" />';
