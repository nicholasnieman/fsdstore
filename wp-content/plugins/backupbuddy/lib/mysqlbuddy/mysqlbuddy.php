...
class pb_backupbuddy_mysqlbuddy {
	...
	private function _dump_php( $output_directory, $tables, $resume_starting_row = 0 ) { //, $base_dump_mode, $additional_excludes ) {
		...
		foreach( $tables as $table_key => $table ) {
			...
			pb_backupbuddy::status( 'details', 'Dumping database table `' . $table . '`. Max rows per select: ' . $max_rows_per_select . '.' );
			if ( $resume_starting_row > 0 ) {
				pb_backupbuddy::status('details', 'Chunked resume on dumping this table. Resuming where left off.' );
				$rows_start = $resume_starting_row;
				$resume_starting_row = 0; // Don't want to skip anything on next table.
			} else {
				$rows_start = 0;
			}
			pb_backupbuddy::status('details', 'Starting at row `' . $resume_starting_row . '`.' );
			
			$output_file = $output_directory . $table . '.sql';
			pb_backupbuddy::status( 'details', 'SQL dump file `' . $output_file . '`.' );
			pb_backupbuddy::status( 'details', 'mysqlbuddy: Running PHP-based database dump.' );
			...
			if ( isset( $output_file ) ) {
				...
				pb_backupbuddy::status( 'details', __('Database SQL dump file size', 'it-l10n-backupbuddy' ) . ': ' . pb_backupbuddy::$format->file_size( $stats['size'] ) );
		...
		if ( false === $results ) {
			if ( $ignore_existing !== true ) {
				...
				if ( 0 == $this->_9010s_encountered ) { // Place a header at the top of this log with some debugging info.
					@unlink( $mysql_9010_log ); // Delete if already exists to avoid multiple logging of entire process.
