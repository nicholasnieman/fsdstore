	...
	class pluginbuddy_dbreplace {
		...
		function bruteforce_table( $table, $olds, $news, $rows_start = '' ) {
			...
			if ( $found_primary_key === false ) {
				pb_backupbuddy::status( 'warning', 'Error #9029: Table `' . $table . '` does not contain a primary key; BackupBuddy cannot safely modify the contents of this table. Skipping migration of this table. (bruteforce_table()).' );
				return true;
			}
