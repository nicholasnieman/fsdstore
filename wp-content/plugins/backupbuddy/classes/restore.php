...
class backupbuddy_restore {
	...
	public function start( $backupFile ) {
		...
		if ( '' == $detectedDatLocation ) {
			return $this->_error( 'Unable to determine DAT file location.' );
		}
	...
	function troubleScan() {
		...
		if ( ! file_exists( ABSPATH . '.htaccess' ) ) {
			...
			if ( strstr( $contents, 'AddHandler' ) ) {
				$trouble[] = 'Warning: An AddHandler directive may have been found in your .htaccess file. This could result in WordPress and PHP not running properly if configured improperly, especially when migrating to a new server. If you encounter problems, remove this line from your .htaccess file.  Solution: Delete this AddHandler line from the .htaccess file. <a target="_new" href="http://ithemes.com/codex/page/BackupBuddy:_Frequent_Support_Issues#.htaccess_404_.2F_Addhandler_Warning">Click here for more information & help.</a>';
			}
		}
		...
		foreach ( (array)$file_array as $line_number => $line) {
			if ( $got_rewrite == true ) { // In a WordPress section.
				if ( strstr( $line, 'END WordPress' ) ) { // End of a WordPress block so stop replacing.
					...
					if ( strstr( $line, 'AddHandler' ) ) { // Has AddHandler
						$message = 'An AddHandler directive may have been found in your .htaccess file. This could result in WordPress and PHP not running properly if configured improperly, especially when migrating to a new server. If you encounter problems, remove this line from your .htaccess file.  Line: "' . $line . '".';
						...
						if ( false !== strstr( $line, 'RewriteRule . ') ) { // RewriteRule, warn user potentially if path may need changed.
							if ( $old_path !== $rewrite_path ) {
								$message = 'User-defined RewriteRule found and WordPress path has changed so this rule MAY need manually updated by you to function properly.  Line: "' . $line . '".';
								pb_backupbuddy::status( 'warning', $message );
							}
						}
					}
