	...
	function wf_add_ups_tracking_info_to_email( $order, $sent_to_admin = false, $plain_text = false ) {
		...
		if( $shipment_id_cs != '' ) {
			...
			echo '<h3>'.$shipping_title.'</h3>';
                        $order_notice =  apply_filters('wf_ups_custom_tracking_message', __( 'Your order is shipped via UPS. To track shipment, please follow the shipment ID(s) ', 'ups-woocommerce-shipping' ), $order, get_locale() );
