...
function jas_claue_wc_add_extra_link_after_cart() {
	...
	echo '<div class="extra-link mt__25 fwsb">';
		if ( ! empty( $size_guide ) ) {
			echo '<a class="cd chp jas-magnific-image" href="' . esc_url( $size_guide ) . '">' . esc_html__( 'Size Guide', 'claue' ) . '</a>';
		}

		if ( ! empty( $message ) ) {
			echo '<a data-type="shipping-return" class="jas-wc-help cd chp ml__20" href="#">' . esc_html__( 'Delivery & Return', 'claue' ) . '</a>';
		}
	echo '</div>';
}
...
function jas_claue_after_add_to_cart_button() {
	if ( class_exists( 'YITH_WCWL' ) ) {
		echo jas_claue_wc_wishlist_button();
	}
}
...
add_action( 'woocommerce_after_add_to_cart_button', 'jas_claue_after_add_to_cart_button' );
 ...
 * @since 1.0.0
 */
function jas_claue_wc_wishlist_button() {
	...
	$output  .= '<div class="yith-wcwl-add-to-wishlist ts__03 mg__0 pa add-to-wishlist-' . esc_attr( $product->get_id() ) . '">';
 ...
 * @since  1.2.2
 */
add_filter( 'body_class','jas_claue_add_to_cart_body_class' );
...
function jas_claue_add_to_cart_body_class($classes) {
	$atc_behavior    = cs_get_option( 'wc-atc-behavior' ) ? cs_get_option( 'wc-atc-behavior' ) : 'slide';
