(function( $ ) {
	...
	function wcPopupAddtocart() {
		...
		// update cart number with custom add to cart and show mini cart with defaul add to cart
		$( document ).ajaxComplete(function( event, xhr, settings ) {
		  if (settings.url.indexOf('claucartf5') > 0) {
		  	refreshMiniCart(false);	
		  }
		  if (settings.url.indexOf('add_to_cart') > 0) { // catch the defaul add to cart ajax
