...
/*Template Name:Download PDF*/
		...
		if(isset($_POST['download_pdf']) || $_SERVER["REQUEST_METHOD"] == "POST" || isset($_POST['print_pdf']) ){
			...
			if($quote_id){
				...
				$se = " select * from ".$wpdb->prefix."internal_builder where quote_id_req= '$quote_id' ";
				...
				foreach($re as $k => $rowe) // = mysql_fetch_object($re))
				{
					...
					$additional_charge 	= $rowe->additional_charge;
					...
					$custom_hrdwr = unserialize(base64_decode($custom_hardware));
				}
				...
				if($product_image == ''){$product_image = '11971'; }
				if($gallery_id == ''){$gallery_id = 'category11969'; }
				if($configure){
					foreach ($configure as $config_key => $config_val) {
						...
						if($builder_val == 'browser' || $builder_val == 'DoorType' || $builder_val == 'OrderStatus'|| $builder_val == 'Price'){
							...
							$return_configuration .= '<tr><td align="left" style="width:300px; font-size:12px">'.$builder_value[0].'</td><td style="font-size:12px">'.$builder_value[1].'</td></tr>';
						}
				...
				// Door Discount
				if($door_before_discount && $door_discount_per != 0){
				...
				// 	$price_configuration .= '<tr><td></td></tr><tr><td align="left" style="width:300px; font-size:11px;"><b>Additional Charges</b></td><td style="font-size:11px;"><b>$'.$additional_charge.'</b></td></tr>';	
				// }

				// Radio
				if($outside_sales == 1){
					$cover_page_img = 'outsidepdf-coverpage.jpg';
				}else if($outside_sales == 2){
					$cover_page_img = 'texaspdf-coverpage.jpg';
				}else{
				...
				$return_string .='<div class="ex-door" >
									<div class="ex-img">
										<img src="'.get_template_directory().'/images/'.$cover_page_img.'" style="min-width: 100%;">
				...
				<div class="row example clear" style="background: #e9ebea none repeat scroll 0 0;padding:15px 20px;">
					<div class="container clear">
						<div class="ex-door1">
							<div class="ex-text" >
								<h3 style="margin:-5px 0 1px 0;font-size: 14px; font-weight:bold; color:#004245;">DOOR EXAMPLE</h3>
