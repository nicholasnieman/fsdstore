			...
			if($quote_id){
				...
				if(trim($StaycleanOption) == 'Yes'){
					if($stayclean_discount_per){
						$stayclean_disprice = trim(str_replace('$','',$stayclean_discount_total));
						$coreprice = round($show_price_diff) - floatval($stayclean_disprice);
						$coreprice = $coreprice - $installation_price;
					}else{
