...
function improved_adding_scripts() {
	...
	if( in_array('single',$classes) && is_single() ){
		...
		if($_GET['mode'] == 'kiosk/'|| $_GET['mode'] == 'kiosk'|| $_GET['mode'] == 'kiosk&'){
			wp_register_script('kioskproduct_js', get_template_directory_uri().'/js/jquery.kioskproduct.js', array('jquery'),'1.1', false);
			wp_enqueue_script('kioskproduct_js');	
			wp_localize_script( 'kioskproduct_js', 'ajax_object', array( 'ajax_url' => admin_url( 'admin-ajax.php' ), 'we_value' => 1234 ) );
			wp_register_script('numpad_js', get_template_directory_uri().'/kiosk/numberpad/jquery.numpad.js', array('jquery'),'1.1', false);
		}
		else if($_GET['mode'] == 'dev_kiosk/'|| $_GET['mode'] == 'dev_kiosk'|| $_GET['mode'] == 'dev_kiosk&'){
			wp_register_script('kioskproduct_js', get_template_directory_uri().'/js/jquery.kioskproduct.js', array('jquery'),'1.1', false);
			wp_enqueue_script('kioskproduct_js');	
			wp_localize_script( 'kioskproduct_js', 'ajax_object', array( 'ajax_url' => admin_url( 'admin-ajax.php' ), 'we_value' => 1234 ) );
			wp_register_script('numpad_js', get_template_directory_uri().'/kiosk/numberpad/jquery.numpad.js', array('jquery'),'1.1', false);
		...
		wp_enqueue_script( 'NiceScroller_js', get_template_directory_uri().'/js/jquery.nicescroll.min.js', array('jquery') );
		wp_enqueue_script( 'jquerybpopup', get_template_directory_uri().'/js/jquery.bpopup.min.js', array('jquery') );
		//wp_enqueue_script( 'jqueryfancybox', get_template_directory_uri().'/js/jquery-1.10.2.min.js', array('jquery') );
		wp_enqueue_script( 'fancybox_pack', get_template_directory_uri().'/js/jquery.fancybox.pack.js', array('jquery') );
		wp_enqueue_script( 'fancybox_buttons', get_template_directory_uri().'/js/jquery.fancybox-buttons.js', array('jquery') );
		wp_enqueue_script( 'fancybox_media', get_template_directory_uri().'/js/jquery.fancybox-media.js', array('jquery') );
		wp_enqueue_script( 'fancybox_thumbs', get_template_directory_uri().'/js/jquery.fancybox-thumbs.js', array('jquery') );
		wp_enqueue_script( 'fancybox_mousewheel_pack', get_template_directory_uri().'/js/jquery.mousewheel.pack.js', array('jquery') );

		wp_enqueue_script( 'owlcarousel', get_template_directory_uri().'/js/owl.carousel.js', array('jquery') );
		
		wp_enqueue_script( 'idle-timer', get_template_directory_uri().'/js/idle-timer.js', array('jquery') );
...
function improved_styles_enqueue() {
	wp_enqueue_style( 'bootstrap', get_template_directory_uri().'/css/basic-css/bootstrap.css' );
	...
	if( is_home() || is_front_page() && in_array('home',$classes) || in_array('page-template-hardware-finishes',$classes) || in_array('page-template-get-started',$classes) || in_array('page-template-get-started-fl-no-builder',$classes) || in_array('page-template-get-startedWORKING',$classes) || in_array('page-template-door-builder',$classes) || in_array('page-template-thank-you',$classes) || in_array('page-main-thank-you',$classes ) || in_array('page-template-contractor',$classes ) || in_array('page-builder',$classes )|| in_array('page-kiosk',$classes ) || in_array('page-template-landing-page',$classes ) ){
		...
		wp_enqueue_style( 'innerstyle', get_template_directory_uri().'/css/style.css' );	
		wp_enqueue_style( 'inner-wrappper', get_template_directory_uri().'/css/inner-wrappper.css' );
	}
...
require_once( get_template_directory().'/cmb2/meta-fields.php' );
	       ...
	       $query = new WP_Query( $args );
	    	...
	    	if ( $query->have_posts() ) {
	    		while ( $query->have_posts() ) {
	    			...
	    			if($style_name != ''){
	    				$thumbnail_image = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'full');
	    				//$image_src = get_stylesheet_directory_uri().'/timthumb.php?src='.$thumbnail_image[0].'&w=205&h=205&q=100';
...
function wc_rebundled_product_towelbarstyle(){
	global $wpdb;
 	if( isset($_REQUEST['hardwrfinish_val']) && isset($_REQUEST['combo_towelbar_size']) && isset($_REQUEST['combo_handle_size']) && isset($_REQUEST['productid']) ){
		...
		$combo_handle_size 	= trim($_REQUEST['combo_handle_size']);
		$productid 			= trim($_REQUEST['productid']);
	    //$towelbar_size1 = get_numerics ($towelbar_size1);
...
function load_custom_wp_admin_script() {
    wp_enqueue_script( 'custom_wp_admin_js', get_template_directory_uri().'/js/admin-custom-jquery.js', array('jquery') );	
}
...
function wc_next_opt_reorder_option(){
  if( isset($_REQUEST['grp_id']) && isset($_REQUEST['option_val']) && isset($_REQUEST['current_opt']) ){
    ...
    //echo $return_arr = json_encode($hold_key_);
  }
  exit();
}
...
function wc_add_sku_to_cart( $product_name, $cart_item, $cart_item_key ){
	...
	//print_r($cart_item['wdm_user_custom_data_value']);
...
function dborg_options_page() {
 ...
 ?>
	...
	<!--End of Discount Mode Options --><?php
        ...
        ?>	
		<form id="discount_hide" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>?page=doorbuilder-options">
			<?php
			...
			foreach( $productconfig as $configkey => $configvalue){ /*Installation Kit*/
				...
				foreach ($configvalue as $configvalue_key => $configvalue_value) {
					...
					foreach ($configvalue_value as $key => $value) {
						...
						//echo $key.'->>'.print_r($serialized_arr);
...
function update_db_price(){
	...
	if( isset($_REQUEST['unique_id']) )
	{
	    ...
	    $additional_charge 			=  $_REQUEST['additional_charge'];
...
function my_filter_head() {
	...
	else if ( is_page( 'builder-thank-you' ) || is_page( 'main-thank-you' || in_array('page-kiosk',$classes) )) {
		remove_action('wp_head', '_admin_bar_bump_cb');
	}
...
add_shortcode('geo_banner', 'geo_banner');
function geo_banner($args = array()) {
    ob_start();
    ?>
...
add_shortcode("geo_email", "geo_email");
function geo_email(){
    ...
    $isFL = $geo->isFL();
    if($isFL){
        $email = '<a href="mailto:contactfl@fsdae.com">contactfl@fsdae.com</a>';
    }else{
        $email = '<a href="mailto:contact@fsdae.com">contact@fsdae.com</a>';
    }
    ob_start();
    echo $email;
    return ob_get_clean();
...
add_shortcode("geo_locations", "geo_locations");
function geo_locations(){
    ...
    $isFL = $geo->isFL();
    if($isFL){
        $locations = '15 Satellite locations across South Florida';
    }else{
        $locations = '71 Satellite locations nationwide';
    }
    ob_start();
    echo $locations;
    return ob_get_clean();
...
add_shortcode("install_easy","install_easy");
function install_easy(){
    ...
    if(!$isFL){
        ob_start();
        ?>
        <p style="color: #1F8F96">
        <img src="https://www.framelessshowerdoors.com/wp-content/uploads/2016/03/logo_13_Final.jpg" alt="" width="288" height="95" /><br>
        Now, for the first time, you the customer have access to our shower door installation trade secrets when you purchase an Installation<strong>EASY</strong><b>™</b> shower door from The Original Frameless Shower Doors company. For all levels of Do-It-Yourselfers, get all the technical support needed from us, the true innovators of glass shower door enclosures, to learn how simple this really is to do! It’s like having one of our installation pro’s right by your side every step of the way! Retail shower door &amp; glass companies are no more than middle men, who will lead you to believe that a custom shower enclosure must be done by an expert. Simply follow our super easy instructions and you too will be a shower door expert!
        <br><br>
        Now are you ready to design your frameless shower door online, get expert advice on how to measure like a professional, place your order and have it ready to be shipped within 48 hours directly from the factory? You will get all the instructions and support on how to install your shower door like one of our own in-house professionals with The Original Frameless Shower Doors guarantee and warranties all implied.
        </p>
        <?php
        return ob_get_clean();
    }
...
add_shortcode("geo_home", "geo_home");
function geo_home(){
    $dir = get_template_directory()."/class/geo_object.php";
    require_once($dir);
    $geo = new geo_object();
    ...
    ob_start();
    if($isFL){
        header("Location: /florida/");
    }
    return ob_get_clean();
}

function fsd_geo_startSession() {
    if(!session_id()) {
        session_start();
    }
}
add_action('init', 'fsd_geo_startSession', 1);
