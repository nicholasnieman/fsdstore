...
$('body').find('.form-right .total_vall .sqft_gls').hide();
...
var glass_sqr_feet;
    ...
    $("#measuremnet-val").click(function(ev) {
        ...
        $('input[type="text"][name="option-qty"]').each(function() {
            ...
            $('body').find('#form_secarea .formLayout form .measurement_values').append(' <input class="builderinput measurmnt_val" type="hidden" value=" Measurement '+ measurmnt_val +'-' +val+ '" name="builder_0[]"> ');
            ...
            if(test_qty == 1 ){
                ...
                var glassoffer = priceString * 5.75;
                var glassoffer = glassoffer.toFixed(2);
    ...
    /* ============ *  Offer Popup * ============ */
        ...
        $('#stay_clean_glass span label').click(function(event) {
          if($(this).text() == 'Yes'){
            ...
            .append(' <input class="builderinput" type="hidden" value=" Stayclean Option- Yes" name="builder[]"> ');
            ...
            $('body').find('#form_secarea .formLayout form .measurement_values')
            .append(' <input class="builderinput" type="hidden" value=" Stayclean Option- No" name="builder[]"> ');
                    ...
                    if(obj.hasOwnProperty(discount_kiosk_pm)){
                        ...
                        if(discount_kiosk){
                            ...
                            if(remove_ins_door_price){
                                jQuery('#kiosk-dis-total').find('span').text('$'+remove_ins_door_price);
                                ...
                                jQuery('#kiosk-wthout_installtion').find('span').text('$'+remove_ins_door);
                                if(instltn_prc < 1){
                                    ...
                                    jQuery('.kiosk-dist-door .custom-info').append('<p id="kiosk-final-price" class="hide-tild-sect kioskdist-tilde-sect"> Total Door price: <span>$'+remove_ins_door+'</span> (with installation)</p>');
                                }
...
if(mode_type == 'builder'){
    ...
    jQuery('#calculate-discount-door').on( "click", function() {
      var discount_stprice   =   jQuery('#dfi_disocunt_txt').attr('value');
      var discount_door      =   jQuery('#door_disocunt_txt').attr('value');
        if(discount_stprice){
          ...
          var total_price = jQuery('body').find('#starburst #price_total').text();
            if(stayclean_price && discount_stprice != 0 && discount_stprice < 70){
                ...
                //var fnlstaycln_price = parseFloat(total_price)+parseFloat(stayclean_price);
        ...
        if(stayclean_price){
            ...
            if(parseFloat(basic_stayclean_price)){
    ...
    $('#download_pdf_crm').on( "click", function() {
        ...
        var outsidesales = jQuery('body').find('input[name="for_outsidesales"]:checked').val();
        if(outsidesales){outside_sales = '1'; }
        else{outside_sales = '0'; }
